...
namespace Coti.Client.Patches
{
    ...
    public class CotiWorldViewPatch : ModulePatch
    {
        ...
        private static void Postfix(Item __0, ref Task<GameObject> __result)
        {
            ...
            __result = DressWhenReady(
                __result,
                item.TemplateId == CotiIds.TplId);
        }
        ...
        public class OnAttachMods : ModulePatch
        {
            protected override MethodBase GetTargetMethod()
            {
                return AccessTools.Method(
                    typeof(PoolManagerClass),
                    nameof(PoolManagerClass.method_3));
            }
            ...
            private static void Postfix( GClass3248 __0, GClass768 __1, ref Task __result)
            {
                ...
                if (collectionView.GameObject == null ||
                    !HasCotiSlot(containerCollection))
                    return;

                __result = DressWhenReady(
                    __result,
                    collectionView.GameObject);
            }
